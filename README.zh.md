...
本包**仅面向 DSH 0.1.7-alpha.1**，且安装器只接受该 `dsh` CLI 版本（见安装小节）。真实账号调用、生产部署与历史迁移为独立事项，不在本包承诺范围内。
...
合成测试保留 pi-ai 0.84.4 owner double 配合目标 adapter；真实 owner/pi-ai 0.85.1 回放、真实账号调用、生产安装与生产历史迁移仍未验证。复制的官方声明/指令许可见 THIRD_PARTY_NOTICES.md。
...
- 安装器要求 PATH 上存在精确版本 `dsh` CLI **`0.1.7-alpha.1`**，所有变更都委托给公开
...
- **`dsh` 缺失、版本不符或 plugin 命令失败时，安装器带指引地失败关闭。** 没有直接改 manifest
  的 fallback。请用 `dsh --version` 核对；这个精确 launcher 版本不放宽插件锁定的
  **0.1.7-alpha.1 宿主包**，也不能用 CLI 版本推断正在运行的 GUI 版本。
- 修正 SDK stream 诊断的拟配套账户包：**`dsh-token-usage` `5.3.0`**
  （仅发布并验收后使用 `github:shaomingbo/dsh-token-usage#v5.3.0`）。兼容旧 owner 仍可执行，
...
Node >= 24，固定公开 DSH `0.1.7-alpha.1`。安装无 lifecycle scripts。
