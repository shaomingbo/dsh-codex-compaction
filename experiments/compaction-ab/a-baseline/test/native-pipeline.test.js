...
test('human /compact executes official lifecycle through native converter, then recompacts restored opaque state', async t => {
  ...
  for (let round = 0; round < 3; round++) {
    ...
    s.append('request/header', { header: { config: agent.options, system: 'Preserve exact task constraints.' }, reason: round ? 'resume' : 'initial' });
    const turn = s.seq;
    ...
    for (let step = 0; step < 2; step++) {
      ...
      s.append('assistant/message', { turn, step, message: assistant(step === 0 ? 'Historical analysis. '.repeat(2000) : 'Latest unchanged tail.') }, { surfaceOp: 'append' });
